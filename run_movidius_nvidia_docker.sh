...
if [ -z "${NCSDK_ID}" ]; then
    ...
    xhost +
    nvidia-docker run -it --privileged --network=host -v /dev:/dev -v ${HOST_DATA_DIR}:${CONTAINER_DATA_DIR}:rw -v /tmp/.X11-unix:/tmp/.X11-unix -e DISPLAY=unix${DISPLAY} -p 14556:14556/udp --name=${NCSDK_NAME} ncsdk_nvidia:ubuntu16.04 bash
else
